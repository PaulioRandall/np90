...
describe('StringReader.js', () => {
	describe('StringReader.accept()', () => {
	...
	describe('StringReader.readWhile', () => {
		...
		test('called twice with regexes that match, returns correct second string', () => {
			const sr = new StringReader('abc123')
			sr.readWhile(/[a-z]/)
			const s = sr.readWhile(/[0-9]/)
	...
	describe('StringReader.skipSpaces', () => {
		...
		test('given mix of whitespace runes, skips all whitespace', () => {
			const sr = new StringReader('\t\f\v\r')
			const s = sr.skipSpaces()
